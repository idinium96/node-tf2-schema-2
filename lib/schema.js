        ...
        if (isCheckPaintKit) {
            ...
            for (const paintkit in paintkits) {
                ...
                if (name.includes(paintkit)) {
                    ...
                    // Standardize https://github.com/TF2Autobot/tf2autobot/pull/394 with this, because you dont care if it is null instead of undefined, even though it should not happen
                    if (!item.effect) {
                        if (item.quality2 === 11) {
                            item.quality = 11;
                            item.quality2 = null;
