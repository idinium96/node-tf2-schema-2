...
class Schema {
    ...
    getItemObjectFromName(name) {
        ...
        if (name.includes('strange')) {
            if (Object.keys(wears).some(w => name.includes(w))) {
                item.quality2 = 11;
            } else {
                item.quality = 11;
            }
        ...
        if (!exception.includes(name)) {
            //Get all qualities
            const qualities = Object.keys(this.raw.schema.qualities).reduce(
                (obj, q) => {
        ...
        //Check for effects
        const isCheckEffect = name.includes('haunted ghosts') ? !Object.keys(wear).some(wear => name.includes(wear)) : true;

        if (isCheckEffect) {
            const effects = this.raw.schema.attribute_controlled_attached_particles.reduce(
                (obj, particle) => {
        ...
        if (name.includes('(paint: ')) {
            ...
            for (const paint in paints) {
                if (name.includes(paint.toLocaleLowerCase())) {
                    name = name.replace(paint.toLocaleLowerCase(), '').trim();
        ...
        if (item.paintkit && name.includes('war paint')) {
            ...
            if (schemaItem.item_class === 'supply_crate') {
                ...
                if (!item.crateseries) {
                    const seriesAttribute = this.raw.items_game.items[schemaItem.defindex]?.static_attrs['set supply crate series'];
