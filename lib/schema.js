...
class Schema {
    ...
    getItemObjectFromName(name) {
        ...
        const weaponEffects = ['hot', 'cool', 'isotope', 'energy orb'];
        const isCheckEffect =
            name.includes('haunted ghosts') // Same name with skin
                ? wears.some(w => name.includes(w)) // first check if include wear
                    ? weaponEffects.some(we => name.startsWith(we)) // if true, check weapon effect
                    : true // else just check
                : true;
