...
class Schema {
     ...
     */
    getPaintByDecimal (decimal) {
        const paintCans = this.raw.schema.items.filter(item => item.name.includes('Paint Can'));

        for (let i = 0; i < paintCans.length; i++) {
            const paint = paintCans[i];
            if (paint.attributes === undefined) continue;
    ...
    getName (item, proper = true) {
        ...
        if (item.paint) {
            name += ` (Paint: ${this.getPaintByDecimal(item.paint)})`
        }
